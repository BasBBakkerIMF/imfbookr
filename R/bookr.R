...
#'
#' @examples
#' plot_line_chart(ifs, "dates", "USA", plot_title="US CPI (2010=100)")
#' plot_line_chart(gdp, "dates", "GDP",
#' plot_title="US GDP", plot_subtitle="Real Gross Domestic Product (SAAR, %Chg)]",
#' plot_caption="Source: Haver")
#'
...
#'
#' @examples
#' plot_bar_chart(gdp, "dates", "GDP", plot_title="US GDP")
#'
...
#'
#' @examples
#' plot_scatter_chart(ifs, "USA", "China", plot_title="GDP: China vs US")
#'
...
#'   plot_subtitle = "Retail vs Inflation (2020–2021)",
#'   caption = "Source: IMF staff estimates"
#' )
plot_dual_axis <- function(data,
  ...
  varL <- rlang::enquo(var_left)
  varR <- rlang::enquo(var_right)

  # Prepare data
  df <- data %>%
    dplyr::select(date, left_series = !!varL, right_series = !!varR) %>%
    dplyr::arrange(date)

  # Compute scaling for dual y-axes
  rng_left  <- range(df$left_series, na.rm = TRUE)
  rng_right <- range(df$right_series, na.rm = TRUE)

  scale_fac <- diff(rng_left) / diff(rng_right)
  offset    <- rng_left[1] - rng_right[1] * scale_fac

  df <- df %>%
    dplyr::mutate(right_scaled = right_series * scale_fac + offset)

  # Build plot
  p <- ggplot2::ggplot(df, ggplot2::aes(x = date)) +
    ggplot2::geom_line(ggplot2::aes(y = left_series), colour = "steelblue", size = 0.8) +
    ggplot2::geom_line(ggplot2::aes(y = right_scaled), colour = "firebrick", size = 0.8) +
    ggplot2::scale_y_continuous(
      name = y_left_lbl,
      sec.axis = ggplot2::sec_axis(~ (. - offset) / scale_fac, name = y_right_lbl)
    ) +
    ggplot2::labs(
      title = plot_title,
      subtitle = plot_subtitle,
    ...
    ggplot2::theme(
      plot.title = ggplot2::element_text(family = primary_font, size = 24, face = "bold", hjust = 0, color = imfblue),
      plot.subtitle = ggplot2::element_text(family = primary_font, size = 18, face = "plain", hjust = 0),
      plot.caption = ggplot2::element_text(family = primary_font, size = 12, hjust = 0),
      axis.title.y = ggplot2::element_text(color = "steelblue", size = 14, family = primary_font),
      axis.title.y.right = ggplot2::element_text(color = "firebrick", size = 14, family = primary_font)
    )
  ...
  # Reapply text and axis title styling in case theme overrides them
  p <- p + ggplot2::theme(
    plot.title = ggplot2::element_text(family = primary_font, size = 24, face = "bold", hjust = 0, color = imfblue),
    plot.subtitle = ggplot2::element_text(family = primary_font, size = 18, face = "plain", hjust = 0),
    plot.caption = ggplot2::element_text(family = primary_font, size = 12, hjust = 0),
    ...
    axis.title.y.right = ggplot2::element_text(
      angle = -90,  # rotate vertically for right side
      color = "firebrick",
      size = 14,
      family = primary_font
    )
  ...
  # Save if plotname is specified
  if (!is.na(plotname) && !is.null(plotname)) {
    save_plot(p, filename = plotname, type = charttype, dpi = dpi, width = width, height = height)
  }
